...
var ArBundles = __toESM(require("@dha-team/arbundles"));
var import_base64url = __toESM(require_base64url2());
var { DataItem } = ArBundles;
...
async function getRawAndId(dataItemBytes) {
  ...
  return {
    id: import_base64url.default.encode(import_buffer.Buffer.from(rawId)),
    raw: dataItem.getRaw()
  };
}
function createANS104Signer(arweaveWallet) {
  const signer = async (create) => arweaveWallet.connect([
    ...
    const res = await getRawAndId(import_buffer.Buffer.from(view));
    return res;
  });
  return signer;
}
function createHttpSigner(arweaveWallet) {
  const signer = async (create) => arweaveWallet.connect([
    ...
    return {
      signature: import_buffer.Buffer.from(view),
      address
    };
  });
  return signer;
